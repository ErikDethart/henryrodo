...
if(_buySell == 0) exitWith {
	...
	if(_type == "Virtual") then {
		...
		["atm","take",_price * _amount] call life_fnc_updateMoney;
	} else {
...
[806, player, format["Obliged a Buy Offer(%1) for %2 units of %3 for a price of $%4 per unit - Offer made by (%5)", _type, _amount, _item, _price, _pid]] remoteExecCall ["ASY_fnc_logIt",2];
[4,_className,_type,_amount,_price,player] remoteExecCall ["KBW_fnc_listing",2]; //Make it easy and use the existing  buy offer function to get us our product
	closeDialog 0;
};
//Made it thus far, indicating it's a buy offer.
switch(_type) do {
	case "Weapon": {
[1,[primaryWeapon player,primaryWeaponItems player],_price,player] remoteExecCall ["KBW_fnc_listing",2];
...
[2,_className,_price,_amount,player] remoteExecCall ["KBW_fnc_listing",2];
		};
	};
};
closeDialog 0;
