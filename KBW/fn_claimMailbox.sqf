...
[803, player, format["Claimed %1 from their Exchange mailbox (ID #%2)", _chosen select 0, _chosen select 2]] remoteExecCall ["ASY_fnc_logIt",2];
		{
			if(_x != "") then {
	...
	case "Virtual": {
		...
		if(_acceptable > _chosen select 1) exitWith {
			...
			life_mailbox = life_mailbox - [_chosen];
[803, player, format["Claimed %1 units of %2 from their Exchange mailbox (ID #%3)", _chosen select 1, _variable, _chosen select 2]] remoteExecCall ["ASY_fnc_logIt",2];
		};
...
[803, player, format["Claimed %1 units of %2 from their Exchange mailbox (ID #%3)", _acceptable, _variable, _chosen select 2]] remoteExecCall ["ASY_fnc_logIt",2];
		hint "You couldn't carry all of what was in your mailbox, so you have withdrawn just as much as you can carry.";
	};
	case "cash": {
		["atm","add",_chosen select 1] call life_fnc_updateMoney;
		life_mailbox = life_mailbox - [_chosen];
[803, player, format["Claimed %1 cash from their Exchange mailbox (ID #%2)", [_chosen select 1] call life_fnc_numberText, _chosen select 2]] remoteExecCall ["ASY_fnc_logIt",2];
	};
};
[life_mailbox,getPlayerUID player] remoteExecCall ["KBW_fnc_updateMailbox",2];
life_mailbox = nil;
