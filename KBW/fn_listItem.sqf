...
_fee = round(_amount * (_price * 0.00));
...
if([false,_var,_amount] call life_fnc_handleInv) then {
	["atm","take",_fee] call life_fnc_updateMoney;
[804, player, format["Listed %1 %4 on the Exchange for $%2 per unit and paid a fee of $%3.", _amount, _price, _fee, _var]] remoteExecCall ["ASY_fnc_logIt",2];
[2,_toSell,_price,_amount,player] remoteExecCall ["KBW_fnc_listing",2];
