...
//_obj remoteExecCall ["ASY_fnc_setIdleTime",2]; //no need for this anymore since we use createVehicle above.
_unit attachTo [_obj,[0,0,_z]];
life_sitting = _obj;
_chair setVariable ["sitting", _unit];//Chairs are local no need to broadcast
...
_chair setVariable ["sitting", nil];//same as above.
