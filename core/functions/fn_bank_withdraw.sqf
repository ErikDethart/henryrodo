...
else
{
	...
	if (life_gov_atm) then {["999999",_val,"take",player] remoteExecCall ["ASY_fnc_updateGangBank",2]; }
	else { 
...
[life_gang,_val,"take",player] remoteExecCall ["ASY_fnc_updateGangBank",2]; 
	};
	life_gang_bank_time = time;
	//[] call life_fnc_sessionUpdate;
	hint format ["You have withdrawn $%1 from your gang bank account",[_val] call life_fnc_numberText];
};
