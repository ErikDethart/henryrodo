...
if (life_wealthPrestige > 0) exitWith {hint "Prestige is no longer used."}; //disable prestige
...
if (_cut < 10) then { _cut = 10; };
_cutTaken = floor (_val * (_cut / 100));
_cash = _val - _cutTaken;
...
systemChat format["You've converted %1 prestige into $%2 after a %3 percent cut.", [_val] call life_fnc_numberText, [_cash] call life_fnc_numberText, _cutTaken];
[259, player, format["Endorsed %1 prestige into $%2 with a cut of %3", _val, _cash, _cutTaken]] remoteExecCall ["ASY_fnc_logIt",2];
life_wealthPrestige = life_wealthPrestige - _val;
["atm","add",_cash] call life_fnc_updateMoney;
