...
while {true} do
{
	...
	_btnRespawn buttonSetAction "life_suicide = true;";
	if ((life_is_arrested) || (!life_session_completed)) then
	{
	...
	{
		if ((player getVariable ["restrained",false]) || (!life_abort_enabled) || (life_isdowned)) then
		{
			_btnRespawn ctrlEnable false;
		...
		else
		{
			...
			{
				if (time - _time < 15 && (call life_adminlevel) < 2) then
				{
					_btnAbort ctrlEnable false;
					_btnRespawn ctrlEnable false;
