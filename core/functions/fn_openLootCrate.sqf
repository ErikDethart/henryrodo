...
life_lootReturn = -1;
[player, _type] remoteExecCall ["ASY_fnc_doOpenLootCrate",2];
while {dialog} do { closeDialog 0; };
waitUntil { life_lootReturn > 0 };

if (life_lootReturn == 911) exitWith {hint "You tried to open a crate without having a key for it!"}; //Array Error
...
{
	[[0,1],"Because you've won a reward you already had, credit has been applied to your account for future key purchases."] call life_fnc_broadcast;
};
