...
if(_varName == "life_honor") exitWith {
	...
	_total = _value * 100;
	switch(true) do {
		case (10 in life_honortalents): {["atm","add",round (0.15 * _total)] call life_fnc_updateMoney; systemChat format["You've received %1 honor and $%2 for an arrest made by your group member",_value,round (0.15 * _total)]};
		case (9 in life_honortalents): {["atm","add",round (0.1 * _total)] call life_fnc_updateMoney; systemChat format["You've received %1 honor and $%2 for an arrest made by your group member",_value,round (0.1 * _total)]};
		case (8 in life_honortalents): {["atm","add",round (0.05 * _total)] call life_fnc_updateMoney; systemChat format["You've received %1 honor and $%2 for an arrest made by your group member",_value,round (0.05 * _total)]};
	};
};
...
switch (_varName) do
{
	case "life_money": { ["cash","set",_value] call life_fnc_updateMoney; };
	case "life_atmmoney": { ["atm","set",_value] call life_fnc_updateMoney; };
