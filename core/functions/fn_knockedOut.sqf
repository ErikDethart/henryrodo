...
if (!life_isdowned) then {
	//[48, player, format["%1 was downed", name player]] remoteExecCall ["ASY_fnc_logIt",2];
	life_isdowned = true;
...
[player,"AcinPercMstpSnonWnonDnon_agony"] remoteExecCall ["life_fnc_animSync",-2];
	...
	while {_downed} do {
		if (!(player getVariable ["restrained",false])) then{
			if (!(_inVehicle)) then{
				if (_time == 5) then {[player,"AinjPpneMstpSnonWrflDnon"] remoteExecCall ["life_fnc_animSync",-2];};
				if (_time == 5) then {
