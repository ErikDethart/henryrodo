...
if (life_wanted > 0) then { _text = _text + format["<img size='1' image='icons\hud_police.paa' /> $%1    ", [life_wanted] call life_fnc_numberText]; };
...
{
	{
		...
		_text = _text + format["<img size='0.5' image='icons\hud_target.paa' /> <t size='1'>%1</t>    ", [_name] call life_fnc_cleanName];
