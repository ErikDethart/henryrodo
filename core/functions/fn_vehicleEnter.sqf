...
if(_vehicle getVariable ["medicsOnly", false] && _position == "driver") then {
	if(side _unit != independent) then {
		_unit action ["GetOut", _vehicle];
"Only medics are licensed to pilot medivac helicopters!" remoteExecCall ["hint", _unit];
	} else {
...
{
[_vehicle,[-0.010813,-0.506166,-0.7]] remoteExecCall ["life_fnc_centerMass",_unit];
};
//"in" remoteExecCall ["life_fnc_fireMode",_unit];
