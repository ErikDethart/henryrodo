...
[player,life_gang,_exp] remoteExecCall ["ASY_fnc_addGangExp",2];
};
...
{
	if (isNil {life_statistics select 5}) then { life_statistics set [5, 0]; };
	life_statistics set[5, (life_statistics select 5) + _add];
