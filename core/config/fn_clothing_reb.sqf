...
//	_ret pushBack ["U_O_CombatUniform_ocamo",nil,1500];
...
if ((_filter == 0) && 29 in life_achievements) then
{
	_ret pushBack ["U_O_OfficerUniform_ocamo","Desert 6 Color",300];
};
...
{
	_ret pushBack ["U_O_PilotCoveralls",nil,6000];
};
...
if ((_filter == 0) && 14 in life_lootRewards) then
{
	_ret pushBack ["U_B_T_Soldier_F",nil,2500];
};

if (_filter == 3) then {
	_ret set [count _ret, ["V_PlateCarrier1_blk",nil,5000]];
	_ret set [count _ret, ["V_PlateCarrier1_rgr_noflag_F",nil,5000]];
	_ret set [count _ret, ["V_PlateCarrier1_tna_F",nil,5000]];

	if ((33 in life_talents) && (!(license_civ_bounty) || (call life_adminlevel) > 1)) then {
		_ret set [count _ret, ["V_HarnessOGL_brn","Suicide vest",80000]];
	};
};
