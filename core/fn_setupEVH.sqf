...
player addEventHandler["getOutMan", {life_in_vehicle = false; [] call life_fnc_updateViewDistance;
	if ((_vehicle isKindOf "Air") && (currentWeapon player == "launch_RPG7_F")) then {
		player action ["WeaponOnBack", player]; //stop them from using RPG exploit
	};
}];
...
spawnHandlerIDR = [["#911", true, true, true], "life_fnc_filterChat"] call aniChatEvents_addEventHandler;
...
//"life_capture_list" addPublicVariableEventHandler {if (life_capture_zone > 0) then { [] call life_fnc_updateCaptureUI; };};//not sure
//"life_bank_lastRobbed" addPublicVariableEventHandler { life_bank_lastRobbed = time; }; gets defined as time on serverside
...
"life_active_turf" addPublicVariableEventHandler
{
	...
	[] spawn
	{
		...
		while {life_active_turf > -1} do
		{
			...
			if (player distance (getMarkerPos format["turf_area_%1", life_active_turf + 1]) < _distance) exitWith
			{
