...
_success = false;
_title = format["Giving transfusion to %1",[name _unit] call life_fnc_cleanName];
...
_delay = (_delay * ([6,"bloodbag"] call life_fnc_infamyModifiers));
...
{
	_unit setDamage _damage;
	titleText[format["You gave %1 a blood transfusion.", [name _unit] call life_fnc_cleanName],"PLAIN"];
[2,format["%1 has given you a blood transfusion.",[name player] call life_fnc_cleanName]] remoteExecCall ["life_fnc_broadcast",_unit];
