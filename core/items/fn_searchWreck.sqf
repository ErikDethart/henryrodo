...
for "_i" from 1 to 10 do { _items set[count _items, "lockpick"]; };
for "_i" from 1 to 10 do { _items set[count _items, "ziptie"]; };
for "_i" from 1 to 4 do { _items set[count _items, "oilp"]; };
for "_i" from 1 to 10 do { _items set[count _items, "doubloon"]; };
for "_i" from 1 to 4 do { _items set[count _items, "pearl"]; };
for "_i" from 1 to 3 do { _items set[count _items, "ruby"]; };
...
while{life_carryWeight < life_maxWeight} do
{
	...
	_random = if (random 9000 < 1) then { "treasure" } else { _items select round (random ((count _items)-1)) };
...
//[6, player, format["Found %1 %2.",_sum,_random]] remoteExecCall ["ASY_fnc_logit",2];
		};
	};
};
