...
[] spawn
{
	_action = player addAction ["Chop Tree", {[cursorObject] spawn life_fnc_chopTree}, nil, 2, true, true, "", ' player distance [(getPos cursorObject) select 0, (getPos cursorObject) select 1, 0] < 4 && {[str cursorObject," t_"] call KRON_StrInStr || [str cursorObject," palm_"] call KRON_StrInStr} '];
