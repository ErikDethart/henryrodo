...
_success = false;
_title = format["Applying splint to %1",[name _unit] call life_fnc_cleanName];
...
_delay = (_delay * ([6,"splint"] call life_fnc_infamyModifiers));

[player,"AinvPknlMstpSnonWnonDnon_medic_1","playNow",3.3] remoteExecCall ["life_fnc_animSync",-2];
while {true} do
{
	uiSleep _delay;
	_cP = _cP + 0.01;
...
{
[false] remoteExecCall ["life_fnc_brokenLeg",_unit];
	titleText[format["You apply a splint to %1's injured leg.", [name _unit] call life_fnc_cleanName],"PLAIN"];
[2,format["%1 has applied a splint to your injured leg.",[name player] call life_fnc_cleanName]] remoteExecCall ["life_fnc_broadcast",_unit];
