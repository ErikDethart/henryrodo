...
{
	{
		if ((_x getVariable ["gangName","omgRanD0M"]) == _name && !isNull(_x getVariable ["gangGroup",objNull]) && count (units (group _x)) < life_groupCap) exitWith { [player] joinSilent (group _x); };
	} foreach allPlayers;
}
else
{
