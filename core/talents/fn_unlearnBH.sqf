...
*/
[] spawn {
	_action = ["Unlearning an honor speciality will cost $5,000 - are you sure?","Unlearn and refund honor","Yes","No"] call BIS_fnc_GUIMessage;
	if(!_action) exitWith {};
	...
	if(_child in life_honortalents) exitWith {hint "This speciality has a child talent which must be unlearned first"};
