...
_word = if(playerSide == civilian) then {["Honor",[life_honor] call life_fnc_numberText]} else {["Prestige",[life_prestige] call life_fnc_numberText]};

if(playerSide == civilian) then {
	if(count life_honortalents > 14) then {_word set[1,"Maxed Out"]};
} else {
	if ((count life_coptalents) isEqualTo (count life_prestigeInfo)) then {_word set[1,"Maxed Out"]};
};

_stats ctrlSetText format["Available %2: %1", _word select 1,_word select 0];
