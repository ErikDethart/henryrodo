...
if (typeName life_clothing_store == "STRING") exitWith {};
...
switch (playerSide) do {
	case west: {
		/*
			...
			_packet set[7, _gear];
		*/
		[3] call life_fnc_sessionUpdatePartial;//Sync licenses
		...
		_packet set[12, 0]; // Doing this for two reasons; #1 - Cops always teleport back to a base anyway, who cares if they get healed doing it? #2 - If a player logged on cop when dead / just respawned this would sync at 100, causing a check in sessionReceive to kill their civ character when they joined later. ~Gnash
	...
	case civilian: {
		...
		*/
		...
		// -Moved to update partial //if (!life_is_arrested) then { [] call life_fnc_civFetchGear; };
		// -Moved to update partial //_packet set[7, if(!alive player) then {[]} else {civ_gear}];
		[4] call life_fnc_sessionUpdatePartial;//Sync gear

		// -Moved to update partial // _packet set[6, life_jail_time];
		[5] call life_fnc_sessionUpdatePartial;//Sync arrest status
		/*
			-Moved to update partial
			...
			_packet set[8, _pos];
		*/
		// Blacklisted position sync zones
		private _noSyncZones = [["life_bank_door", 55], ["fed_reserve", 185], ["capture_label_1", 200], ["capture_label_2", 200], ["capture_label_3", 200], ["capture_label_4", 200]];
		// Check to sync position
		if (!(alive player) || (({(player distance (getMarkerPos (_x select 0))) <= (_x select 1)} count _noSyncZones) isEqualTo 0)) then {
			// Sync player position
			[6] call life_fnc_sessionUpdatePartial;
		};


		// -Moved to update partial //_packet set[19, round (life_pain * 100)];
		// -Moved to update partial //_packet set[20, life_brokenLeg];
		// -Moved to update partial //_packet set[9, alive player];
		// -Moved to update partial //_packet set[12, round ((damage player) * 100)];
		/*
			-Moved to update partial
			if ((player getVariable["can_revive",-1000]) > time) then {
				_packet set[30, (player getVariable["can_revive",time]) - time];
			} else {
				_packet set[30, 0];
			};
		*/
		[7] call life_fnc_sessionUpdatePartial;//Sync health values


		// -Moved to update partial //_packet set[15, life_gang];
		// -Moved to update partial //_packet set[16, life_gang_rank];
		[8] call life_fnc_sessionUpdatePartial;//Sync gang information


		// -Moved to update partial //_packet set[31, [player getVariable ["customTexture", ["",""]], player getVariable ["customTextureBP", ["",""]]]];
		[9] call life_fnc_sessionUpdatePartial;//sync custom texture

		[11] call life_fnc_sessionUpdatePartial;//infamy level/talents

		if ((player getVariable["parole",-1000]) > time) then {
			_packet set[32, [(player getVariable["parole",time]) - time, player getVariable["paroleViolated",false]]];
		} else {
			_packet set[32, [0,false]];
		};
...
_name = [profileName,1] call life_fnc_cleanName;
_send set [0, _name];
...
_send remoteExec ["DB_fnc_update",2];