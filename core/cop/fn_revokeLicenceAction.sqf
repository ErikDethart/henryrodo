...
switch (_type) do {
	case "drivers": {
		...
		hint format["Your Drivers License was revoked by %1.",[name _cop] call life_fnc_cleanName];
		systemChat format["Your Drivers License was revoked by %1.",name _cop];
	};
	case "bounty": {
		license_civ_bounty = false;
		hint format["Your Bounty Hunter License was revoked by %1.",[name _cop] call life_fnc_cleanName];
