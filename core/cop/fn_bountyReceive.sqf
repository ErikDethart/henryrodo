...
if(playerSide == civilian && player == _arrester) then {
	...
	systemChat format["You've received %1 honor for an arrest!",[round (_original / 50)] call life_fnc_numberText];
...
if (_val > life_arrest_cap) then { _val = life_arrest_cap; };
if (_total > life_arrest_cap) then { _total = life_arrest_cap; };
