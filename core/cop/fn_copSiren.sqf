...
_vehicle remoteExec ["life_fnc_copLights",-2];
		};
	};
	...
	if (isNull _sirenLogic) exitWith {};
	if(_forcedPhaser) then {
		[_vehicle, _sirenLogic, "SirenPhaser", _phaserLength, false, player] remoteExec ["life_fnc_playCopSiren",-2];
	} else {
		// otherwise, play your normal first siren sound
		[_vehicle, _sirenLogic, _sirenType, _sirenDuration, false, player] remoteExec ["life_fnc_playCopSiren",-2];
		...
		if(_vehicle getVariable "sirenDual") then {
			...
			[_vehicle, _sirenLogic, _dualType, _dualDuration, _sameDual, player] remoteExec ["life_fnc_playCopSiren",-2];
		};
	};
