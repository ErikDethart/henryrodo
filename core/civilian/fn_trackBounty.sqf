...
_retarget = [_this,2,false,[false]] call BIS_fnc_param;
_bad = false;
if (_uid == "") then {
...
_unit = objNull;
_count = 0;
_trackTime = (life_bounty_tracked select 0);
...
if(_unit distance getMarkerPos "respawn_west" < 750 || _unit distance getMarkerPos "courtroom" < 75 || isNull _unit) then {_unit = player; _bad = true; hint "The target was dead or participating in a trial, the tracking circle has been placed at your position so that you can attempt to track them again later."};
_searchTime = switch (true) do
{
	...
	case (6 in life_honortalents && 126 in life_talents): {120};
	case (5 in life_honortalents && 126 in life_talents): {150};
	default { 180 };
};

if (7 in life_honortalents && 126 in life_talents) then {
	_count = ({time - _x < 120} count life_bounty_tracked);
};

if (!_isNew && (_count >= 2)) exitWith {
	_tracks=[];
	{
		if (time - _x < 120) then {_tracks pushBack _x};
	}forEach life_bounty_tracked;
	hint format["You can only track two unique targets every 2 minutes, you can track a target again in %1 seconds ", round(_searchTime - (time - (selectMin _tracks)))];
};

if (!_isNew && ((time - _trackTime) < _searchTime) && !_silent && !(7 in life_honortalents && 126 in life_talents)) exitWith { hint format["You can only track a target once every %1 minutes, you can track again in %2 seconds ", _searchTime/60,round(_searchTime - (time - (selectMax life_bounty_tracked)))]};
...
life_track_radius = switch (true) do
{
	...
	case (4 in life_honortalents && 126 in life_talents): {175};
	case (3 in life_honortalents && 126 in life_talents): {210};
	case (2 in life_honortalents && 126 in life_talents): {250};
...
if(11 in life_honortalents && 126 in life_talents) then {
	bounty_pos = _pos;
...
if (!_isNew) then {
	if !(7 in life_honorTalents) then {
		life_bounty_tracked set [0,time];
		} else {
		life_bounty_tracked set [_bountyIndex, time];
		};
};
_bounty set [5, _pos];
...
{
	{
		if (_x != player) then {[true, _uid, _bounty select 0, _pos, life_track_radius] remoteExecCall ["life_fnc_setGroupTrackMarker", _x]; };
...
if (!_silent) then {
	...
	[[0,2], format[_message + ".",[_bounty select 0] call life_fnc_cleanName, player distance _pos]] call life_fnc_broadcast;
};
if (playerSide == civilian) then {
	if !(_retarget) then {
		...
		if(_index != -1) then {
			_newData = [_uid,_pos];
			life_bounty_list set[_index,_newData];
		} else {
			_newData = [_uid,_pos];
			life_bounty_list set[count life_bounty_list,_newData];
		};
