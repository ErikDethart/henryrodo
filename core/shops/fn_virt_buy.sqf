...
_price = [_type,_price,_qty,true] call life_fnc_calcPrice;
...
if (life_shop_type == "rebel") then
{
	...
	["cash","take",(_price * _amount)] call life_fnc_updateMoney;
};
if (_exit) exitWith {};
...
//[92, player, format["Purchased %1 %2 for %3", _amount, _name, (_price * _amount)]] remoteExecCall ["ASY_fnc_logIt",2];
		if (_qty > -1) then
		{
