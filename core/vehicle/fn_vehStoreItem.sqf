...
if(!(_ctrl in ["oilu","oilp","pickaxe"]) && (typeOf life_trunk_vehicle) in ["B_Truck_01_fuel_F","C_Truck_02_fuel_F","C_Van_01_fuel_F"]) exitWith {hint "This vehicle can only hold oil and oil accessories!";};
...
if(_ctrl == "money") then
{
	...
	["cash","take",_num] call life_fnc_updateMoney;
...
//[70, player, format["Put %1 %2 in trunk of %3", _num, _ctrl, typeOf life_trunk_vehicle]] remoteExecCall ["ASY_fnc_logIt",2];
