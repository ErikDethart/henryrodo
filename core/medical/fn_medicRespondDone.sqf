...
if(!isNull life_medic_target && life_medic_target == _responder) then {
  ["atm","add", _travelBonus] call life_fnc_updateMoney;
  ...
  if(_responder != player) then {
    ...
    {
      _travelBonus = round(((getPos player) distance (life_medic_startpos))*0.02);
      life_medic_target = objNull;
    } else {
      _travelBonus = 0;
    };
    ["atm","add", (life_revive_fee + _travelBonus)] call life_fnc_updateMoney;
