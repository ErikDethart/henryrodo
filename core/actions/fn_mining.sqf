...
while{life_carryWeight < life_maxWeight - 3} do
{
	...
	_random = switch (true) do
	{
		...
		case (!life_is_arrested && floor (random (15000 / _yield)) < 1): { "treasure" };
		case (!life_is_arrested && floor (random (500 / _yield)) < 1): { "diamondf" };
		case (!life_is_arrested && floor (random (150 / _yield)) < 1): { ["lootcrate1","lootcrate2","lootcrate3"] call BIS_fnc_selectRandom; };
