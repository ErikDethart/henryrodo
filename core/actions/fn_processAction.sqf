...
_itemInfo = switch (_type) do
{
	...
	case "drilledoil": {[["oilu"],"oilp",10,"Processing Oil",2];};
	...
	case "timber": {[["timber"],"plank",0,"Processing Pine Timber",0]};
	case "timberl": {[["timberl"],"plankl",0,"Processing Olive Timber",0]};
	case "timberh": {[["timberh"],"plankh",0,"Processing Hardwood Timber",0]};
	...
	case "crank" : {[["ephedrau","phos","hydro"],"crank",0,"Mixing Ingredients",0]};
	case "crankp" : {[["crank"],"crankp",0,"Cooking Crank",0]};
...
//No crank from inside a building
_nBuilding = nearestBuilding player;
if ((_type in ["crank","crankp"]) && (player distance _nBuilding <= 10)) exitWith { hint "You're too close to a building to be messing with a mobile lab."};
...
_hasLicense = true;
if (_type in ["oil","drilledoil","diamond","copper","iron","sand","salt","cement"]) then { _hasLicense = missionNamespace getVariable (([_type,0] call life_fnc_licenseType) select 0); };
...
while{true} do {
	...
	if ((_type in ["crank","crankp"]) && (player distance _vehicle > 5)) exitWith {};
...
if ((_type in ["crank","crankp"]) && (player distance _vehicle > 5)) exitWith { hint "You're messing around with dangerous stuff, you should stand still."; 5 cutText ["","PLAIN"]; life_is_processing = false; life_action_in_use = false;};
...
//Removes the old items
{
	if(!([false,_x,_oldVal] call life_fnc_handleInv)) exitWith {
		5 cutText ["","PLAIN"]; life_is_processing = false; life_action_in_use = false; _exit = true;
	};
} foreach _oldItem;

if (_exit) exitWith {[35, player, format["Attempted exploit (dropping ingredients) while processing %1", _type]] remoteExecCall ["ASY_fnc_logIt",2];};
...
if ((_type == "crankp") && !(27 in life_talents)) then { _reduce = true; };
if (_type in ["timberl","timberh"]) then { _reduce = true; };
if (_reduce) then
{
...
if (_capture) then
{
	...
	if (((_point select 1) != life_gang) || (_point select 2) < 1) then
	{
		_oldVal = switch (true) do
		{
			...
			case (_type == "oil"): { floor(_oldVal * 0.95) };
...
if (_type != "plate" && _type != "rock") then
{
	...
	else
	{
		switch (_type) do
		{
			...
			case "drilledoil": {life_experience = life_experience + (_oldVal * 3)};
...
else
{
	if (_hasLicense) then {
		...
		["cash","take",_cost] call life_fnc_updateMoney;
	};
};

life_is_processing = false;
//[36, player, format["Processed %1 into %2", _oldVal, _itemName]] remoteExecCall ["ASY_fnc_logIt",2];