...
if(count _fish == 0) exitWith {titleText["Didn't catch any fish...","PLAIN"]; life_net_dropped = false;};
{
	if(_x isKindOf "Fish_Base_F") then
	{
		...
		if (random 12000 < 1) then
		{
			...
			if([true,"treasure",1] call life_fnc_handleInv) then
			{
				titleText["YOU FOUND SOME TREASURE!","PLAIN"];
			};
		}
		else
		{
			...
			{
				_type = ["lootcrate1","lootcrate2","lootcrate3"] call BIS_fnc_selectRandom;
