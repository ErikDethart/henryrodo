...
[1,format["A store is being robbed by %1!", [name player] call life_fnc_cleanName]] remoteExecCall ["life_fnc_broadcast",-2];
};
...
[player, _unit] remoteExecCall ["ASY_fnc_robStore",2];
...
if (_robSuccess) then
{
	...
	[] spawn { uiSleep 300; life_use_atm = true; };
}
else
{
