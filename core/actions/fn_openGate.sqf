...
_gate = nearestObjects [player, ["Land_BarGate_F"],20] select 0;
...
if (!([5, true] call life_fnc_policeRequired) && !(life_bank_inProgress)) exitWith {hint "There aren't enough officers to do this!"};
