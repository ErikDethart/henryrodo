...
{
	if (life_bank_lastRobbed > (time - 1800)) exitWith {_die = true; hint format["Security is high due to a recent robbery and you cannot hack this vault.  Try again in %1 minute(s).", ceil ((life_bank_lastRobbed - (time - 1800))/60)]};
};
...
while{true} do
{
	if(animationState player != "acts_supportteam_front_kneelloop") then {
		player playMoveNow "acts_supportteam_front_kneelloop";
[player,"acts_supportteam_front_kneelloop"] remoteExecCall ["life_fnc_animSync",-2];
	};
	uiSleep _delay;
	_cP = _cP + 0.01;
...
if (_success) then
{
	...
	if (!life_bank_inProgress) then
	{
		...
		if ((floor random _chance) == 0) then {[player] remoteExec ["ASY_fnc_fedAlarm",2]; };
	};
};
