...
if (_success) then
{
	...
	if ((_dis/floor(time - life_dp_time)) > 60) then {[922,player,format["Delivered package from %1 to %2 (%7m) in %3 seconds (~%6m/s). Using %4 and gaining $%5",life_dp_start,_dp,round((time - life_dp_time)),typeOf (life_dp_vehicle),[_price] call life_fnc_numberText,round(_dis/floor(time - life_dp_time)),_dis]] remoteExecCall ["ASY_fnc_logIt",2];};
}
else
{
