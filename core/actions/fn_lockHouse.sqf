...
{
	if (!(_house in life_houses) && ((call life_coplevel) < 4) && (life_swatlevel < 3) && (_buildingID != life_gang)) exitWith {};
...
//[23, player, format["Set locks for house at %1 to %2", position _house, _isLocked]] remoteExecCall ["ASY_fnc_logIt",2];

//[_buildingID, _isLocked, position _house] remoteExecCall ["DB_fnc_updateHouse",2];
	};
};

if (_isLocked == 1) then { _soundFile = "lock"; }
else { _soundFile = "unlock"; };
[player, _soundFile, 10] remoteExecCall ["life_fnc_playSound",-2];
