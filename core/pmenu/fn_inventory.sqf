...
if (_isPhone) then { [] call life_fnc_p_updateMenu; };
...
if (_newLoad && !life_virt_open && !_isPhone) then
{
	...
	_but3 ctrlCommit 0;
	_but3 ctrlAddEventHandler ["ButtonClick", "[player,false,life_in_vehicle] spawn life_fnc_dropItems;"];
...
{ if(player distance _x < 10) then {_near_units set [count _near_units,_x];};} foreach allPlayers;
{
	...
	{
		_near lbAdd format["%1 - %2",[name _x] call life_fnc_cleanName, side _x];
...
{
	_inv lbAdd format["%1's scalp", (_x select 2)];
	_inv lbSetData [(lbSize _inv)-1,""];
	_inv lbSetPicture [(lbSize _inv)-1,format["icons\%1.paa","skull"]];
