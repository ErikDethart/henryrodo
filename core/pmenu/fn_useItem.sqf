...
switch (true) do
{
	...
	case (_item == "campfire"):
	{
		...
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			...
			_fire addAction["Cook Raw Meat",life_fnc_cookMeat,cursorObject,-1,false,false,"",'!isNull cursorObject && (player distance cursorObject) < 4 && (typeOf cursorObject) == "Land_Campfire_F" && inflamed cursorObject && (life_inv_chicken > 0 || life_inv_dog > 0 || life_inv_sheep > 0 || life_inv_goat > 0 || life_inv_manflesh > 0) '];
