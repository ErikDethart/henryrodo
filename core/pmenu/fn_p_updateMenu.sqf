...
_base = (sqrt(_max) + _max ^ 2) * 18;
_percent = 0;
if (_max >= life_maxTalents) then { _next = _base; }
else { _next = (sqrt(_max+1) + (_max+1) ^ 2) * 18; };
...
_ttr = if (isNil "serverStartTime") then {round((14400 - serverTime) / 60)} else {round((serverCycleLength - (serverTime - serverStartTime)) / 60)};
_time ctrlSetStructuredText parseText format ["<t align='center' shadow='0' size='0.65'>%1:%2 [%3 mins]</t>", _hour, _min, _ttr];
