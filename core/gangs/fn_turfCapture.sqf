...
_exit = false;
if ((currentWeapon player) in ["","Binocular","hgun_Pistol_Signal_F"]) exitWith { hint "You can't take turf without a weapon!"};
...
if (_success) then
{
//Sanitizing this for the hint that pops when the turf is captured. ~Gnash
	life_turf_captor = [[profileName] call life_fnc_cleanName, life_gang_name, life_gang];
