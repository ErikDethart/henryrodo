...
if(time - life_last_sold < 5) exitWith {hint "You cannot rapidly sell vehicles from your garage!"};
...
{
	if (((_x select 0) == _shop) && (_x select 1) == _vehicle) exitWith { _index = _forEachIndex; };
...
[_vid,_pid,_price,life_garage_type, _vehicleInfo select 0] remoteExecCall ["ASY_fnc_vehicleDelete",2];
life_last_sold = time;
