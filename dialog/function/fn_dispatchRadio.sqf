...
{
	if (_chan > -1) then { hint format["Added %1 to the APD dispatch communications channel.", [name _unit] call life_fnc_cleanName]; }
	else { hint format["Removed %1 from the APD dispatch communications channel.", [name _unit] call life_fnc_cleanName]; };
};

[_unit, _chan, true, _flush] remoteExecCall ["ASY_fnc_manageRadio",2];
