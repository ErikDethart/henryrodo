...
KRON_StrLeft = {
	params ["_string","_length",["_startIndex",0,[0]]];
	private _out = _string select [_startIndex, _length];
	_out
};
...
KRON_StrIndex = {
	...
	if (_initialIndex < 1) then {
		...
		private _tempString = _string select [_haystack, _initialIndex];
...
KRON_StrInStr = {
	private _out = (([_this select 0,_this select 1] call KRON_StrIndex)>-1);
 	_out
};
...
KRON_StrLower = {
	private _out=toLower(_this select 0);
	_out
};
