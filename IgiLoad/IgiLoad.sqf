...
if (isnil "IL_Variables") then
{
	...
	IL_Supported_Quadbike = ["I_Quadbike_01_F", "C_Quadbike_01_F", "O_Quadbike_01_F", "B_G_Quadbike_01_F", "B_Quadbike_01_F", "CAF_Quadbike_OD", "CAF_Quadbike_AR", "Marinir_Quadbike_01_FG", "B_mas_mar_Quadbike_01_F","C_Kart_01_F","C_Kart_01_Fuel_F","C_Kart_01_Blu_F","C_Kart_01_Red_F","C_Kart_01_Vrana_F","C_Kart_01_green_F","C_Kart_01_orange_F","C_Kart_01_white_F","C_Kart_01_yellow_F","C_Kart_01_black_F"];
	IL_Supported_Supply_Crate = [""];
	IL_Supported_Veh_Ammo = ["Land_CargoBox_V1_F"];
	IL_Supported_Barrel = [""];
	IL_Supported_Tank = [""];
	IL_Supported_Rubberboat = ["I_Boat_Transport_01_F", "O_Boat_Transport_01_F", "B_G_Boat_Transport_01_F", "B_Boat_Transport_01_F", "C_Rubberboat", "O_Lifeboat", "B_Lifeboat", "Marinir_duck_base_F", "B_mas_mar_Boat_Transport_01_F","C_Scooter_Transport_01_F","C_Boat_Transport_02_F"];
...
//	END IL_Cargo_Para
	...
	IL_Do_Load =
	{
		...
		if (_v getVariable["labMode", false]) exitWith { hint "You cannot do this while your vehicle is setup to cook.";_v setVariable["can_load", true, true]; };
		...
		{
			{
				if ((locked _x < 2 || playerSide == west) && !(_x getVariable["labMode", false])) then
				{
				...
				{
					_crank = (_x getVariable["labMode", false]);
					_msgc = "as it's locked";
					if (_crank) then {_msgc = "as it is setup to cook crank"};
					[_v, Format ["""%1"" could not be loaded %2.", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"),_msgc], true] call IL_Vehicle_Chat;
				};
				if (_done) exitWith {};
			} forEach (_obj_lst);
		}
